...
|---|---|---|---|---|---|---|---|---|---|
|R|S|P|D|Linear Regression|Baseline predictions|1. Simple to understand and explain. 2. It seldom overfits. 3. Using L1 & L2 regularization is effective in feature selection. 4. Fast to train.|1. sensitive to outliers. 2. You have to work hard to make it fit nonlinear functions.|weights, bias, and learning rate| Mean Square Error (MSE) $$L(y, \hat{y}) = \frac{1}{M}\sum_{i=1}^{M}(\hat{y_i}-y_i)^2$$, or  $$L(y, x, w) = \frac{1}{M}\sum_{i=1}^{M}(\hat{y_i}-(w^Tx_i+b))^2$$|
